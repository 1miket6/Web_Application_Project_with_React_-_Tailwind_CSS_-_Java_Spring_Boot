...
export default function Contact() {
  ...
  const labelStyle =
    "block text-lg font-semibold text-primary dark:text-dark mb-2";
  const textFieldStyle =
    "w-full px-4 py-2 text-base border rounded-md transition border-primary dark:border-dark focus:ring focus:ring-dark dark:focus:ring-lighter focus:outline-none text-primary dark:text-dark bg-darkbg dark:bg-normalbg placeholder-normalbg dark:placeholder-darkbg";
  return (
    <div className="max-w-[1152px] min-h-[852px] mx-auto px-6 py-8 font-primary bg-normalbg dark:bg-darkbg">
      {/* Page Title */}
      <PageTitle title="Contact Us" />
      {/* Contact Info */}
      <p className="max-w-[768px] mx-auto mt-8 text-darkbg dark:text-normalbg mb-8 text-center">
...
export async function contactAction({ request, params }) {
  ...
  try {
    ...
    throw new Response(
      error.response?.data?.errorMessage || error.message || "Failed to submit your message. Please try again.",{ status: error.status || 500 }
    );
  }
}
